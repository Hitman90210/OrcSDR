...
> **[Recent project screen captures](docs/screenshots/) — current UI from this fork.**
> The images further down this page came from upstream and predate the GMRS band,
> the channel scanner, and the weather channel picker. That folder has up-to-date
> captures taken from real hardware, each named for what it shows.

Weather radio, CB, GMRS/FRS and marine VHF are finished and channelised — weather has a WX1–WX7 picker over the seven NOAA channels, CB the standard 40, GMRS/FRS the 30 channels of the 462/467 MHz plan (1–22 plus the R15–R22 repeater inputs), and marine VHF the 40-channel US monitoring set including 16, 13, 09 and 22A. All four carry a **SCAN** button that steps the channel list, stops on a busy channel, and resumes ~2.5 s after it goes quiet; individual channels can be locked out so the sweep steps over them. The weather band also decodes **SAME/EAS alert headers**, so an alert reports its event code and the counties it covers rather than just playing a voice. GMRS/FRS is receive-only: it names the channels so you can tell what you are hearing, and transmitting on GMRS needs an FCC licence. POCSAG pager receive is also present. AM and the shared “browse the whole tuner” shell run on the same Home workspace and are not given their own section below.
...
Companion is optional. There is a LAN web console intended for something like an Android TV. It shows receiver telemetry, streams audio independently of the Tab5 speaker mute, and offers basic tuning, volume, and screen controls. On ADS-B it carries up to 16 nearest targets, plots relative positions only after the receiver location is configured, and preserves valid zero-valued altitude, speed, heading, and vertical-rate measurements. It never exposes saved passwords or coordinates, but it has no TLS or web login, so enable it only on a trusted LAN. Phone connection is optional. Bluetooth speaker audio is not available on the Tab5 C6. **OrcSDR remains fully usable with no phone, BLE, GPS, or extra host software.**
