...
cd "$repo_root"
python3 tests/test_data_catalog.py
build_dir="$(mktemp -d)"
...
sources=(tests/p25_core_tests.cpp tests/p25_config_tests.cpp apps/orcsdr-tab5/ui/p25_decoder_core.cpp apps/orcsdr-tab5/ui/p25_voice.cpp apps/orcsdr-tab5/ui/p25_config.cpp)
...
common=(-std=c++17 -Wall -Wextra -Werror -pedantic -ffunction-sections -fdata-sections -Iapps/orcsdr-tab5/ui -Icomponents/mbelib)

g++ "${common[@]}" -O2 "${sources[@]}" "${objects[@]}" -Wl,--gc-sections -lm -o "$build_dir/p25_core_tests"
...
g++ "${common[@]}" -O1 -g -fsanitize=address,undefined -fno-omit-frame-pointer \
  "${sources[@]}" "${san_objects[@]}" -Wl,--gc-sections -lm -o "$build_dir/p25_core_tests_sanitized"
