...
function Invoke-AmBroadcastTest {
  ...
  try {
    ...
    # A single status read raced the stream coming up: the state flips to
    # STREAMING before the first IQ block lands, so poll for bytes as well.
    $streamDeadline = [DateTime]::UtcNow.AddSeconds(30)
    do {
      $driver = Get-DriverStatus
      if ($driver.State -eq 'STREAMING' -and $driver.Bytes -gt 0) { break }
    ...
    [void](Open-Ui 'HOME' 'AM')
    [void](Open-Ui 'FM' 'FM')
    # Same race on the way back: a fixed 500 ms was not enough for the FM
    # capture to restart on a busy boot, so wait for real bytes instead.
