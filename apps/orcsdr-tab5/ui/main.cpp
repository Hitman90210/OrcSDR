...
constexpr RfBandGuide kRfBandGuide[] = {
    ...
    {156000000, 157500000, kMarineDefaultHz, RtlBand::marine, "MARINE VHF", "VHF / 40 US marine channels", true},
...
constexpr const char* kRfQuickLabels[] = {
    ...
    "HAM 2M", "MARINE VHF", "NOAA WX", "HAM 70CM", "P25 PHASE I", "GMRS / FRS",
    "LORA 915",
    "ADS-B 1090", "POCSAG"};
static_assert(std::size(kRfQuickLabels) == 15);
...
// Runtime state only -- never persisted -- so this may be reordered freely,
// unlike dashboards::Id. am_presets is upstream's; fm_seek and channel_scan
// are this fork's.
...
// Set when a hold is no longer wanted -- locking out the channel the sweep
// is parked on should release it immediately, not strand the receiver on a
// channel it has just been told to skip.
std::atomic<bool> channel_scan_resume_requested{false};
...
// Normal connects always pause SDR traffic. This opt-in flag exists only for
// the authenticated coexistence diagnostic that measures the unsafe live-load
// baseline; it is consumed once and never set by the touchscreen UI.
std::atomic<bool> wifi_connect_live_diagnostic_requested{false};
...
const char* rtl_band_name(RtlBand band) {
  switch (band) {
    ...
    case RtlBand::marine: return "MARINE";
...
bool rtl_band_from_name(const char* name, RtlBand* out_band) {
  ...
  if (strcmp(name, "MARINE") == 0) { *out_band = RtlBand::marine; return true; }
...
// VHF aviation voice is amplitude-modulated everywhere -- ICAO Annex 10 fixes
// double-sideband AM for the 118-137 MHz band -- so a BROWSE tune inside it has
// to demodulate AM even though BROWSE is otherwise an NFM band.
//
// This was a real gap rather than a refinement: the home screen ships an
// AIRBAND tile described as "VHF aviation voice", dashboard_for_band() routes
// 118-137 MHz to it, and open_dashboard() tunes BROWSE to 121.5 MHz -- and
// every one of those paths then ran an FM discriminator, which on an AM carrier
// produces noise rather than speech. BROWSE is the only band whose demodulator
// depends on where it happens to be tuned.
constexpr uint32_t kAirbandMinHz = 118000000;
constexpr uint32_t kAirbandMaxHz = 137000000;
...
static_assert(!band_demodulates_am(RtlBand::browse, 146520000), "2 m calling stays NFM");
static_assert(!band_demodulates_am(RtlBand::fm, 121500000), "only BROWSE is position-dependent");
...
uint32_t rtl_band_default_frequency(RtlBand band) {
  switch (band) {
    // Upstream restored this in the band-restore chain, which this fork replaced
    // with the switch below. Without it here an AM restore silently lands on the
    // band default instead of where the user left the dial.
    case RtlBand::am: return orcsdr::am::saved_frequency();
    case RtlBand::wx: return kRtlWxHz;
    case RtlBand::cb: return kCbDefaultHz;
    case RtlBand::gmrs: return kGmrsDefaultHz;
    case RtlBand::marine: return kMarineDefaultHz;
...
uint32_t rtl_filter_default_hz(RtlBand band, uint32_t frequency_hz) {
  ...
  // Airband channels are 25 kHz with roughly 6 kHz of audio, so the AM width
  // fits them; the NFM default below is far too wide for a DSB-AM carrier.
  ...
  while (true) {
    ...
    if (!block.custom_rate && !orcsdr::visualizer::channel_audio_active() &&
      ...
      if (block.band == RtlBand::cb) {
        ...
        // The BROWSE half of that test is the airband: read the tuned
        // frequency rather than caching it, so a hot retune out of 118-137 MHz
        // switches the demodulator on the very next block.
        demodulate_am(block.data, block.bytes, block.audio_scale);
      } else if (block.band == RtlBand::gmrs || block.band == RtlBand::marine) {
  ...
  while (true) {
    ...
    if (g_rtl != nullptr && g_rtl_device_ready.load(std::memory_order_acquire) &&
      ...
      g_stream_audio_scale =
          (band == RtlBand::wx || band == RtlBand::browse ||
           band == RtlBand::gmrs || band == RtlBand::marine)
              ? 12000.0f
...
bool disconnect_wifi() {
  ...
  wifi_connect_live_diagnostic_requested.store(false, std::memory_order_release);
  wifi_connecting = false;
...
void poll_wifi() {
  ...
  if (wifi_connect_requested.exchange(false, std::memory_order_acq_rel)) {
    const bool live_diagnostic =
        wifi_connect_live_diagnostic_requested.exchange(false, std::memory_order_acq_rel);
    // Issue #66: normal Hosted connects always take the radio-pause window.
    // The only exception is an explicit authenticated diagnostic command.
    start_wifi_connection(!live_diagnostic);
  }
  if (!wifi_station_ready) return;
...
bool scan_retune(uint32_t frequency_hz, void*) {
  if (active_scan == ActiveScan::fm_presets || active_scan == ActiveScan::am_presets ||
      active_scan == ActiveScan::fm_seek || active_scan == ActiveScan::channel_scan)
    reset_spectrum_renderer();
...
bool start_channel_scan_pass(uint32_t now, uint32_t after_hz) {
  ...
  // Locked-out channels are dropped here rather than skipped during the sweep,
  // so the engine never dwells on one at all.
  static uint32_t rotated[kMaxPlanChannels];
  size_t count = 0;
  for (size_t i = 0; i < plan.count; ++i) {
    const size_t index = (first + i) % plan.count;
    if (channel_is_locked(channel_scan_band, index)) continue;
    rotated[count++] = plan.channels_hz[index];
  }
  // Everything locked out is a user error, not a crash: say so and refuse
  // rather than starting an engine pass with an empty list.
  if (count == 0) {
    Serial.printf("RTL_CHANNEL_SCAN_UNAVAILABLE band=%s reason=all_locked\n",
                  rtl_band_name(channel_scan_band));
    return false;
  }
  const orcsdr::scan::Plan scan_plan{orcsdr::scan::Mode::channel_list, rotated,
                                     count, 0, 0, kChannelScanDwellMs, false};
...
void service_channel_scan(uint32_t now, const orcsdr::scan::Callbacks& callbacks) {
  ...
  if (channel_scan_requested.exchange(false, std::memory_order_acq_rel)) {
    ...
    channel_scan_resume_requested.store(false, std::memory_order_relaxed);
  ...
  if (channel_scan_holding.load(std::memory_order_acquire)) {
    if (channel_scan_resume_requested.exchange(false, std::memory_order_acq_rel)) {
      const uint32_t from_hz = channel_scan_hit_hz;
      channel_scan_quiet_since_ms = 0;
      if (start_channel_scan_pass(now, from_hz)) {
        rtl_stream_ui_refresh_pending.store(true, std::memory_order_release);
        Serial.printf("RTL_CHANNEL_SCAN resume after_hz=%lu reason=locked_out\n",
...
orcsdr::home::Snapshot home_dashboard_snapshot(bool demo) {
  ...
  snapshot.step_hz = rtl_ui_band == RtlBand::fm       ? rtl_fm_step_hz
                     ...
                     : rtl_ui_band == RtlBand::marine ? 25000
  ...
  snapshot.channel =
      ...
      : rtl_ui_band == RtlBand::marine
          ? static_cast<uint8_t>(marine_channel_index(rtl_ui_frequency_hz) + 1)
          : 0;
...
orcsdr::dashboards::Id dashboard_for_band(RtlBand band, uint32_t frequency_hz) {
  ...
  switch (band) {
    ...
    case RtlBand::marine: return Id::marine;
    ...
    case RtlBand::browse:
      ...
      if (frequency_is_airband(frequency_hz)) return Id::airband;
...
void open_dashboard(orcsdr::dashboards::Id id) {
  ...
  switch (id) {
    ...
    case Id::marine: band = RtlBand::marine; frequency = kMarineDefaultHz; break;
  ...
  switch (action.kind) {
    ...
    case orcsdr::settings::ActionKind::catalog_check:
      if (orcsdr::catalog::state().busy) {
        ...
        // Offline, ask anyway but do not stop the radio for it: the request
        // refuses itself and leaves "Connect Wi-Fi before downloading" on the
        // panel, which is the whole point of the call.
    ...
    case orcsdr::settings::ActionKind::catalog_install:
      if (orcsdr::catalog::state().busy) {
        ...
        // Same as the check above: refuse without pausing the radio.
...
void load_state() {
  ...
  if (preferences.isKey("last_band")) {
    ...
    if (stored_band == RtlBand::fm || stored_band == RtlBand::am ||
        ...
        stored_band == RtlBand::gmrs || stored_band == RtlBand::marine ||
        stored_band == RtlBand::lora ||
      ...
      // Restore the selected band's own frequency rather than clamping the
      // previous band's dial. LoRa additionally restores its persisted region
      // and slot before choosing the frequency.
      if (stored_band == RtlBand::lora) {
        load_lora_config();
...
bool queue_local_rtl_listen(RtlBand band, uint32_t frequency_hz,
  ...
  if (band != rtl_ui_band) {
    rtl_filter_bandwidth_hz.store(rtl_filter_default_hz(band, frequency_hz),
                                  std::memory_order_relaxed);
  }
#if !RTL_USE_LEGACY_USB
  ...
  if (persist_navigation)
    append_journal(band == RtlBand::am       ? "sdr_am"
                   ...
                   : band == RtlBand::marine ? "sdr_marine"
...
constexpr UiDocScreen kUiDocScreens[] = {
    ...
    {"marine.radio", "live,demo"},
    {"marine.scope", "live,demo"},
    {"marine.capture", "live,demo"},
...
bool ui_doc_self_check() {
  if (static_cast<uint8_t>(orcsdr::fm::View::count) != 5 ||
      ...
      ui_doc_prefix_count("marine.") != static_cast<uint8_t>(OrcTool::Count) ||
...
[[gnu::noinline]] bool ui_doc_render_generic_band(const char* screen_id, bool demo) {
  ...
  static constexpr GenericBand bands[] = {{"am.", RtlBand::am, kRtlAmDefaultHz},
      ...
      {"marine.", RtlBand::marine, kMarineDefaultHz},
  ...
  for (const auto& entry : bands) {
    ...
    rtl_filter_bandwidth_hz.store(rtl_filter_default_hz(entry.band, entry.frequency),
                                  std::memory_order_relaxed);
    g_orc_tool.store(static_cast<uint8_t>(tool), std::memory_order_release);
    if (demo) {
...
void process_command(char* command) {
  ...
  if (strcmp(command, "RTL_WIFI_CONNECT_SAVED") == 0 ||
      ...
      strcmp(command, "RTL_WIFI_CONNECT_SAVED LIVE") == 0) {
    if (!authenticated) {
    ...
    const bool live_diagnostic = strcmp(command, "RTL_WIFI_CONNECT_SAVED LIVE") == 0;
    wifi_connect_live_diagnostic_requested.store(live_diagnostic, std::memory_order_release);
    wifi_connect_requested.store(true, std::memory_order_release);
    Serial.printf("RTL_WIFI_CONNECT_QUEUED saved_profile=0 mode=%s\n",
                  live_diagnostic ? "live" : "pause");
    return;
  }
  ...
  if (strcmp(command, "RTL_HELP") == 0) {
    ...
    Serial.println("RTL_WIFI_CONNECT_SAVED [PAUSE|LIVE]|DISCONNECT - paused connect; LIVE is diagnostic only");
    Serial.println("SET_WIFI <ssid_hex> <pass_hex> <hmac> - signed slot-0 provisioning (auth)");
    Serial.println("RTL_TUNE <BAND> <HZ>           - tune band+freq (auth)");
    Serial.println("    BAND=FM|AM|WX|CB|GMRS|MARINE|LORA|BROWSE|ADSB|P25|POCSAG");
  ...
  // Lockout is addressed by channel NAME, not index: "R15" and "22A" are what
  // the dashboard shows, and an index would be a second numbering to get wrong.
  if (strncmp(command, "RTL_CHANNEL_LOCK ", 17) == 0 ||
      strncmp(command, "RTL_CHANNEL_UNLOCK ", 19) == 0) {
    const bool lock = command[12] == 'L';
    const char* name = command + (lock ? 17 : 19);
    if (!authenticated) {
      Serial.println("RTL_CHANNEL_LOCK_ERROR auth_required");
      return;
    }
    const ChannelPlan plan = channel_plan_for(rtl_ui_band);
    if (plan.channels_hz == nullptr) {
      Serial.println("RTL_CHANNEL_LOCK_INVALID channelized bands only (CB|GMRS|WX|MARINE)");
      return;
    }
    size_t index = SIZE_MAX;
    for (size_t i = 0; i < plan.count; ++i) {
      if (strcasecmp(channel_name_for(rtl_ui_band, i), name) == 0) { index = i; break; }
    }
    if (index == SIZE_MAX) {
      Serial.printf("RTL_CHANNEL_LOCK_INVALID band=%s unknown channel \"%s\"\n",
                    rtl_band_name(rtl_ui_band), name);
      return;
    }
    // Locking the channel the sweep is parked on should release it, not leave
    // the receiver sitting on a channel it has been told to skip.
    (void)set_channel_locked(rtl_ui_band, index, lock);
    if (lock && channel_scan_active.load(std::memory_order_acquire) &&
        channel_index_for(rtl_ui_band, rtl_ui_frequency_hz) == index)
      channel_scan_resume_requested.store(true, std::memory_order_release);
    Serial.printf("RTL_CHANNEL_LOCK_OK band=%s channel=%s locked=%d total_locked=%u\n",
                  rtl_band_name(rtl_ui_band), channel_name_for(rtl_ui_band, index),
                  lock ? 1 : 0,
                  static_cast<unsigned>(channel_locked_count(rtl_ui_band)));
    return;
  }
  if (strcmp(command, "RTL_CHANNEL_LOCK_LIST") == 0) {
    const ChannelPlan plan = channel_plan_for(rtl_ui_band);
    if (plan.channels_hz == nullptr) {
      Serial.println("RTL_CHANNEL_LOCK_INVALID channelized bands only (CB|GMRS|WX|MARINE)");
      return;
    }
    Serial.printf("RTL_CHANNEL_LOCK_LIST band=%s count=%u locked=%u\n",
                  rtl_band_name(rtl_ui_band), static_cast<unsigned>(plan.count),
                  static_cast<unsigned>(channel_locked_count(rtl_ui_band)));
    for (size_t i = 0; i < plan.count; ++i)
      Serial.printf("RTL_CHANNEL_LOCK_ENTRY channel=%s frequency_hz=%lu locked=%d\n",
                    channel_name_for(rtl_ui_band, i),
                    static_cast<unsigned long>(plan.channels_hz[i]),
                    channel_is_locked(rtl_ui_band, i) ? 1 : 0);
...
void setup() {
  ...
  Serial.println(orcsdr::am::self_check() ? "RTL_AM_DASHBOARD_SELF_CHECK_OK"
                                          : "RTL_AM_DASHBOARD_SELF_CHECK_FAIL");
  Serial.println(orcsdr::am_finder::self_check() ? "RTL_AM_FINDER_SELF_CHECK_OK"
                                                 : "RTL_AM_FINDER_SELF_CHECK_FAIL");
  ...
  // Kept in the ternary form rather than upstream's rewrite, which prints
  // the _FAIL line and then the _OK line unconditionally -- so a failing
  // check reported both, and any log scraper matching _OK saw a pass.
  Serial.println(orcsdr::pocsag::self_check() && orcsdr::pocsag::Decoder::self_check() &&
                         orcsdr::pocsag_store::Table::self_check()
                     ? "RTL_POCSAG_SELF_CHECK_OK"
                     : "RTL_POCSAG_SELF_CHECK_FAIL");
  ...
  Serial.println(orcsdr::lora::self_check() && orcsdr::lora_log::self_check()
                     ? "RTL_LORA_DASHBOARD_SELF_CHECK_OK"
