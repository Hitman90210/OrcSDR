...
constexpr Descriptor kEntries[] = {
    ...
    {Id::gmrs, "GMRS / FRS", "462/467 MHz handheld and repeater channels", Category::audio, true},
...
bool self_check() {
  ...
  return loaded && moved && std::size(kEntries) == 16 && find(Id::rf_lab) != nullptr &&
         find(Id::am) != nullptr && find(Id::wifi_analysis) != nullptr &&
         find(Id::pocsag) != nullptr && find(Id::gmrs) != nullptr;
}
