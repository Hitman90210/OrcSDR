...
bool self_check() {
  // Three peaks against a two-slot capacity, so the fixture proves the
  // selector keeps the *strongest* two (-60 and -50) and not merely the first
  // two it walks past (-70 and -60), then re-sorts them into index order.
  ...
  return static_cast<uint8_t>(View::count) == 4 &&
         ...
         kTabsY < 720 && audio_header::self_check() &&
          ...
          // A flat band has no candidates: the prominence gate must reject
          // every level rather than hand the dial three ties to pick from.
